...
- Indentation: use tabs with sizes:
  - 4-wide for JS/MJS
  - 2-wide for HBS, JSON, HTML, CSS, LESS

# V2 Sheets Guide

ApplicationV2 - The Application class is responsible for rendering an HTMLElement into the Foundry Virtual Tabletop user interface.
...
// Keep in mind that the order of `parts` _does_ matter
...
A static PARTS object.
A static TABS object.
Data in the context (constructed in \_prepareContext) that describes each tab.
A \_preparePartContext that sets context.tab appropriately.
The standard tab handlebars template (included in 1.)
...
/\*_ @type {Record<string, foundry.applications.types.ApplicationTabsConfiguration>} _/
static TABS = {
primary: {
