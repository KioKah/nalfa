...
- `module/sheets/nalfaItemSheet.mjs`: ItemSheetV2-based v2 sheet.
  - Minimal sheet; currently renders a template that is mostly a FontAwesome dice playground.
  - Contains scaffolding for ActiveEffect controls (handler is currently commented out).
...
scripts). These are not referenced by current runtime code.
...
- For editable rich text, use `{{editor ...}}` and set `target="system.description"`.
